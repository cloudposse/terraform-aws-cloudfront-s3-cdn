...
resource "aws_s3_bucket_policy" "default" {
  ...
  #   cloudfront_origin_access_identity_iam_arn = local.cloudfront_origin_access_identity_iam_arn
  # })

  # a hacky replacement for a string version of templatefile() which is 1:1 equivalent with template_file data source
  policy = replace(replace(replace(local.iam_policy_document,
      "\${origin_path}", coalesce(var.origin_path, "/")),
      "\${bucket_name}", local.bucket),
      "\${cloudfront_origin_access_identity_iam_arn}", local.cloudfront_origin_access_identity_iam_arn
  )
